...
public:
	...
	void SetBlob( const clPtr<Blob>& Ptr ) { FBlob = Ptr; }
private:
	clPtr<Blob> FBlob;
};
...
public:
	...
	{
		const int MAX_LINE_WIDTH = 16384;
		char Buf[ MAX_LINE_WIDTH + 1 ];

		const ubyte* C = &( MapStream()[FPosition] );
		char* Out       = Buf;
		char* End       = Buf + MAX_LINE_WIDTH;

		uint64 Sz = GetSize();

		while ( FPosition < Sz && Out < End )
		{
			FPosition++;
...
{
public:
	MemFileWriter( clPtr<Blob> Container ): FPosition( 0 ), FMaxSize(), FContainer( Container ), FFileName( "" ) {}
