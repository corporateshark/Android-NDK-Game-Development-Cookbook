...
static ALCboolean alsa_reset_playback( ALCdevice* device )
{
	...
	if ( i >= 0 && ( i = psnd_pcm_hw_params_set_format( data->pcmHandle, p, format ) ) < 0 )
	{
		...
		{
			case 1:
				device->Format = AL_FORMAT_MONO_FLOAT32;
				break;

			case 2:
				device->Format = AL_FORMAT_STEREO_FLOAT32;
				break;

			case 4:
				device->Format = AL_FORMAT_QUAD32;
				break;

			case 6:
				device->Format = AL_FORMAT_51CHN32;
				break;

			case 7:
				device->Format = AL_FORMAT_61CHN32;
				break;
		...
		if ( ( i = psnd_pcm_hw_params_set_format( data->pcmHandle, p, SND_PCM_FORMAT_FLOAT ) ) < 0 )
		{
			...
			if ( ( i = psnd_pcm_hw_params_set_format( data->pcmHandle, p, SND_PCM_FORMAT_S16 ) ) < 0 )
			{
				switch ( aluChannelsFromFormat( device->Format ) )
				{
					...
					case 7:
						device->Format = AL_FORMAT_61CHN8;
						break;
